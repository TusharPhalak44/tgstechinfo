-- Drop and recreate tags table with clean data
...
-- Extract clean tags from comma-separated format
...
FROM (
    ...
    AND SUBSTRING_INDEX(SUBSTRING_INDEX(tags, ',', n.n), ',', -1) NOT LIKE '[%'
    AND SUBSTRING_INDEX(SUBSTRING_INDEX(tags, ',', n.n), ',', -1) NOT LIKE '"%'
) as extracted_tags
...
-- Extract clean tags from JSON array format
INSERT INTO tags (name, slug, usage_count)
SELECT 
